...
from app.schemas.invoice import InvoiceCreate, InvoiceResponse, InvoiceList, ValidationResult
...
async def create_invoice(
    ...
    company = db.query(Company).filter(
        Company.id == invoice_data.company_id
    ).first()
    ...
    logger.info(f"Invoice {invoice.invoice_number} created by user {current_user.email}")
...
async def validate_fa3(
    ...
    if not company:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Firma nie została znaleziona"
        )
    ...
    logger.info(f"FA(3) validation completed for invoice {invoice_data.invoice_number} by user {current_user.email}")
