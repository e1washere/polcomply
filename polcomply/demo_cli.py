...
def format_table_output(errors: list[ValidationError], filename: str) -> str:
    ...
    for error in errors:
        ...
        code = (
            error.code[:18] + "..."
            if error.code and len(error.code) > 18
            else (error.code or "-")
        )
        message = (
            error.message[:38] + "..." if len(error.message) > 38 else error.message
        )
...
def main():
    ...
    parser.add_argument(
        "--schema", help="Path to XSD schema file (auto-resolve if not provided)"
    )
    ...
    if not schema_path or not schema_path.exists():
        print(
            "❌ FA-3 schema not found. Please provide --schema or place FA-3.xsd in schemas/"
        )
        return 1
